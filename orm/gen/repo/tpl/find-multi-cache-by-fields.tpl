...
{{- if .haveDeletedAt }}
// FindMultiUnscopedCacheBy{{.upperFields}} 根据{{.upperFields}}查询多条数据（包括软删除），并设置缓存
func ({{.firstTableChar}} *{{.upperTableName}}Repo) FindMultiUnscopedCacheBy{{.upperFields}}(ctx context.Context, {{.fieldAndDataTypes}}) ([]*{{.dbName}}_model.{{.upperTableName}}, error) {
	resp := make([]*{{.dbName}}_model.{{.upperTableName}}, 0)
