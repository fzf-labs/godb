...
func ({{.firstTableChar}} *{{.upperTableName}}Repo) FindMultiUnscopedCacheByCondition(ctx context.Context, conditionReq *condition.Req) ([]*{{.dbName}}_model.{{.upperTableName}}, *condition.Reply, error) {
	...
	cacheKey := {{.firstTableChar}}.cache.Key(Cache{{.upperTableName}}UnscopedByConditionPrefix)
	cacheField := conditionReq.ConvertToCacheField()
	cacheValue, err := {{.firstTableChar}}.cache.FetchHash(ctx, cacheKey,cacheField, func() (string, error) {
		result, conditionReply, err := {{.firstTableChar}}.FindMultiUnscopedByCondition(ctx, conditionReq)
		if err != nil {
			return "", err
		}
		tmp.Result = result
