...
func ({{.firstTableChar}} *{{.upperTableName}}Repo) FindOneUnscopedCacheBy{{.upperFields}}(ctx context.Context, {{.fieldAndDataTypes}}) (*{{.dbName}}_model.{{.upperTableName}}, error) {
	resp := new({{.dbName}}_model.{{.upperTableName}})
	cacheKey := {{.firstTableChar}}.cache.Key(Cache{{.upperTableName}}UnscopedBy{{.upperFields}}Prefix,{{.cacheFieldsJoin}})
	cacheValue, err := {{.firstTableChar}}.cache.Fetch(ctx, cacheKey, func() (string, error) {
		dao := {{.dbName}}_dao.Use({{.firstTableChar}}.db).{{.upperTableName}}
