...
{{- if .haveDeletedAt }}
// DeleteMultiUnscopedCacheBy{{.upperFieldPlural}} 根据{{.lowerFieldPlural}}删除多条数据，并删除缓存
func ({{.firstTableChar}} *{{.upperTableName}}Repo) DeleteMultiUnscopedCacheBy{{.upperFieldPlural}}(ctx context.Context, {{.lowerFieldPlural}} []{{.dataType}}) error {
	dao := {{.dbName}}_dao.Use({{.firstTableChar}}.db).{{.upperTableName}}
