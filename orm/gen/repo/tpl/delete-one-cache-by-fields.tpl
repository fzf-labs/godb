...
{{- if .haveDeletedAt }}
// DeleteOneUnscopedCacheBy{{.upperFields}} 根据{{.lowerField}}删除一条数据，并删除缓存
func ({{.firstTableChar}} *{{.upperTableName}}Repo) DeleteOneUnscopedCacheBy{{.upperFields}}(ctx context.Context, {{.fieldAndDataTypes}}) error {
	dao := {{.dbName}}_dao.Use({{.firstTableChar}}.db).{{.upperTableName}}
